...
if ($action === 'conversations') {
    ...
    // Hugo (admin_id=1) ve mensajes sin admin_id O con su ID. Otros ven solo sus propios.
    $condition = $adminId === 1 ? "(admin_id IS NULL OR admin_id = $adminId)" : "(admin_id = $adminId)";
    ...
    $sql = "
        SELECT
            ...
            l.whatsapp  AS phone,
            l.nicho     AS nicho,
            (SELECT body FROM wa_messages WHERE lead_id = l.id AND $condition ORDER BY created_at DESC LIMIT 1) AS last_msg,
            (SELECT created_at FROM wa_messages WHERE lead_id = l.id AND $condition ORDER BY created_at DESC LIMIT 1) AS last_ts,
            (SELECT COUNT(*) FROM wa_messages WHERE lead_id = l.id AND direction = 'in' AND leido = 0 AND $condition) AS unread,
        ...
        FROM leads l
        WHERE EXISTS (SELECT 1 FROM wa_messages WHERE lead_id = l.id AND $condition)
        UNION ALL
        SELECT
            NULL          AS lead_id,
            ...
            (SELECT body FROM wa_messages m2 WHERE m2.from_phone = m.from_phone AND m2.lead_id IS NULL AND $condition ORDER BY m2.created_at DESC LIMIT 1) AS last_msg,
        ...
        FROM wa_messages m
        WHERE lead_id IS NULL AND $condition
        GROUP BY from_phone
